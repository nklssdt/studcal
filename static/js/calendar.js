...
function makeCalendar(year, month) {
    ...
    if(month < 10) {
        var fmonth = '0' + month;
    } else { var fmonth = month; }
    
    for (let i = 1; i <= getChek.daysInMonth; i++) {
        if (i === 1) {
            if (month === 6) { getChek.firstDay += 1; }
            var div = '<li date="' + year + '-' + fmonth + '-0' + i + '" style="grid-column-start: ' + getChek.firstDay + ';">1</li>';
        } else {
            if (i < 10) {
                var fi = '0' + i;
            } else { var fi = i; }
            ...
            var div = '<li class="'+ setactive +'" date="' + year + '-' + fmonth + '-' + fi + '" >' + i + '</li>'
        }
